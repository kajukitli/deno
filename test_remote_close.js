  ...
  if (url.pathname === '/remote-close') {
    ...
    if (req.headers.get('upgrade') === 'websocket') {
      ...
      socket.onopen = () => {
        ...
        setTimeout(() => {
          console.log(`Server: Sending close frame with code=${code}, reason="${reason}"`);
          socket.close(parseInt(code), reason);
        }, 10);
      };
      
      socket.onclose = (event) => {
        console.log(`Server: WebSocket closed with code=${event.code}, reason="${event.reason}"`);
      };
...
async function testRemoteClose() {
  ...
  try {
    const wss = new WebSocketStream('ws://localhost:8081/remote-close?code=4222&reason=remote');
    console.log('Client: Opening WebSocket...');
    
    await wss.opened;
    console.log('Client: WebSocket opened');
    
    console.log('Client: Sending local close frame with code=4111, reason="local"');
    wss.close({ closeCode: 4111, reason: 'local' });
    
    const { closeCode, reason } = await wss.closed;
    console.log(`Client: WebSocket closed with code=${closeCode}, reason="${reason}"`);
    
    if (closeCode === 4222 && reason === 'remote') {
      console.log('✅ SUCCESS: Remote code and reason were used correctly!');
    } else {
      console.log('❌ FAILURE: Expected code=4222, reason="remote", but got code=' + closeCode + ', reason="' + reason + '"');
    }
